...
const getAllCategory = () => {
    return async dispatch => {
        ...
        const res = await axios.get(`https://adflipkartbackend.herokuapp.com/api/category/getcategory`);
        console.log(res);
        if (res.status === 200) {
...
export const addCategory = (form) => {
    return async dispatch => {
        ...
        try {
            const res = await axios.post(`https://adflipkartbackend.herokuapp.com/api/category/create`, form);
            if (res.status === 201) {
                dispatch({
...
export const updateCategories = (form) => {
    return async dispatch => {
        ...
        const res = await axios.post(`https://adflipkartbackend.herokuapp.com/api/category/update`, form);
        if (res.status === 201) {
...
export const deleteCategories = (ids) => {
    return async dispatch => {
        ...
        const res = await axios.post(`https://adflipkartbackend.herokuapp.com/api/category/delete`, {
            payload: {
                ids
            }
        });
